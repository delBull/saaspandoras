    ...
    <main className="bg-[#060606] text-white selection:bg-white/10">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-[#060606] px-6">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.4 }} className="text-center max-w-4xl">
          ...
          <p className="text-xs uppercase tracking-[0.5em] text-amber-500/80 mb-4">Libro VII</p>
          <h1 className="text-5xl md:text-7xl font-thin tracking-tight text-white mb-6">Growth & Expansion</h1>
          <p className="text-zinc-500 text-sm font-light max-w-md mx-auto mb-16">
            Estrategia de Crecimiento Institucional, Roadmap Geográfico y Access as a Service (AaaS)
          </p>

          <div className="border border-white/[0.05] p-6 rounded-xl bg-white/[0.01] text-left space-y-4 max-w-2xl mx-auto">
            <p className="text-zinc-300 text-sm font-light">Fases de Escalabilidad:</p>
            <p className="text-zinc-500 text-xs">1. **Fase 1:** Institucionalización del IOM y Asset Register.</p>
            <p className="text-zinc-500 text-xs">2. **Fase 2:** Apertura de Pandoras USA Operations LLC para levantamiento de capital internacional.</p>
            <p className="text-zinc-500 text-xs">3. **Fase 3:** Consolidación LatAm (México, Colombia, Brasil via Blokko).</p>
          </div>
        </motion.div>
      </section>

      <section className="py-24 flex flex-col items-center justify-center bg-[#060606]">
        <p className="text-[9px] uppercase tracking-[0.6em] text-zinc-800">Pandoras Holdings · Libro VII · Confidencial · {new Date().getFullYear()}</p>
        <div className="mt-6 flex gap-6 flex-wrap justify-center">
          ...
          <a href={`/libros/libro-viii?token=${token}`} className="text-xs text-zinc-700 hover:text-zinc-400 transition-colors">Libro VIII: Institutional Doctrine →</a>
        </div>
      </section>
    </main>
  );
