...
export class AddOnGovernanceService {
  ...
  static async requestInstallation(addonId: string, context: ControlPlaneContext) {
    ...
    await db.insert(hermesAddonInstallations).values(newInstallation);
    ...
    await db.insert(hermesAddonAudit).values(audit);
  ...
  static async configureAddOn(installationId: string, configuration: Record<string, unknown>, context: ControlPlaneContext) {
    const [installation] = await db.select().from(hermesAddonInstallations).where(eq(hermesAddonInstallations.id, installationId)).limit(1);
  ...
  static async suspendAddOn(installationId: string, context: ControlPlaneContext) {
    ...
    await db.update(hermesAddonInstallations).set(updateData).where(eq(hermesAddonInstallations.id, installationId));
    await db.insert(hermesAddonAudit).values(this.generateAuditRecord(
