...
export class IntegrationKeyService {
     ...
     */
    static async ensureKeyForProject(
        projectId: number, 
        ...
        // 2. Generate new key
        const { key, hash, fingerprint } = this.generateKey(env, type);
