...
export function ProjectForm({ project, isAdmin: propsIsAdmin }: ProjectFormProps) {
  ...
  const defaultValues: Partial<ProjectFormData> = {
    ...
    businessCategory: (project?.businessCategory as "residential_real_estate" | "commercial_real_estate" | "tech_startup" | "renewable_energy" | "art_collectibles" | "intellectual_property" | "other") ?? "other",
    ...
    socials: project?.socials ? JSON.parse(project.socials as string) : { twitter: "", discord: "", telegram: "", linkedin: "" },
    targetAmount: Number(project?.targetAmount ?? 0),
    totalValuationUsd: Number(project?.totalValuationUsd ?? 0),
    tokenType: (project?.tokenType as "erc20" | "erc721" | "erc1155") ?? "erc20",
    ...
    yieldSource: (project?.yieldSource as "rental_income" | "capital_appreciation" | "dividends" | "royalties" | "other") ?? "other",
    lockupPeriod: project?.lockupPeriod ?? "",
    fundUsage: project?.fundUsage ?? "",
    teamMembers: project?.teamMembers ? JSON.parse(project.teamMembers as string) : [],
    advisors: project?.advisors ? JSON.parse(project.advisors as string) : [],
    tokenDistribution: project?.tokenDistribution ? JSON.parse(project.tokenDistribution as string) : { team: 0, investors: 0, community: 0, treasury: 0, other: 0 },
    ...
    status: (project?.status as "pending" | "approved" | "live" | "completed" | "rejected") ?? "pending",
