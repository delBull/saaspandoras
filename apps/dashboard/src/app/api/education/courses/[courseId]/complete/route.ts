...
export async function POST(
  ...
  try {
    ...
    // If not already completed, mark as completed in DB
    ...
    // Fire gamification event ONLY if it's the first time
    if (!isAlreadyCompleted) {
        try {
    ...
    return NextResponse.json({
        ...
        message: isAlreadyCompleted ? "Revisión finalizada" : "¡Curso completado exitosamente!",
        xpAwarded: isAlreadyCompleted ? 0 : course.xpReward,
        creditsAwarded: isAlreadyCompleted ? 0 : course.creditsReward,
