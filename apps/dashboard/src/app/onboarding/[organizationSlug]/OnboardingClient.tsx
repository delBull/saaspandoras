        ...
        <div className="bg-[#0C0C12] p-3.5 sm:p-5 border-b border-white/5 shrink-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-white tracking-tight flex items-center gap-2 truncate">
              ...
              <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full border border-indigo-500/30 truncate max-w-[150px] sm:max-w-none">{organizationName}</span>
            </h1>
            <p className="text-[11px] sm:text-xs text-zinc-400 mt-0.5">Hermes OS Discovery Concierge</p>
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-2.5 w-full sm:w-auto">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span className="text-[11px] sm:text-xs font-medium text-emerald-400 uppercase tracking-wider">En Línea</span>
            </div>
            <button
              onClick={handleBulkActivate}
              disabled={isActivating}
              className="bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white text-xs font-bold px-3.5 py-2.5 sm:py-2 rounded-xl flex items-center justify-center gap-1.5 shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-50 min-h-[40px] sm:min-h-0"
            >
