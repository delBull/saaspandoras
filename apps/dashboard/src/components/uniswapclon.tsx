...
export default function UniswapClon() {
  ...
  // Handler robusto: solo DEX steps swap/approval
  const handleSwap = useCallback(async () => {
    ...
    try {
      ...
      let sawSell = false;
      // Solo steps "approval", "sell" o "swap": nunca "pay" ni "checkout"
      const steps = (prepared?.steps ?? []).filter(
        (step: any) => {
          const act = ((step as any)?.action || (step as any)?.type || "").toLowerCase();
          return (
            act === "approval" ||
            act === "sell" ||
            act === "swap" ||
            // Fallback for steps that might not have action/type but have transactions
            (!("action" in (step as any)) &&
              !("type" in (step as any)) &&
              Array.isArray((step as any).transactions) &&
              (step as any).transactions.length > 0)
          );
        },
      );
      ...
      if (!steps.length) {
        setSwapError(
          "No hay ruta DEX directa para este par en este momento (solo onramp/fiat disponible).",
        );
        setSwapping(false);
        return;
      }
      ...
      for (const step of steps) {
        ...
        if (act === "approval") {
          ...
          setApprovalDone(false);
          for (const tx of (step as any).transactions ?? []) {
            try {
              await sendTx(tx);
              setApprovalPending(false);
      ...
      if (!sawSell && !swapSuccess) {
        setSwapError(
          "No hay ruta DEX directa ni transacción de swap posible.",
        );
      } else {
