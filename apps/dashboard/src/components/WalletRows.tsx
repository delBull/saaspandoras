...
export function WalletRows() {
  ...
  useEffect(() => {
    const fetchBalances = async () => {
      ...
      let ethBalance = 0;
      try {
        const bal = await getWalletBalance({ client, address, chain: config.chain });
      ...
      let usdcBalance = 0;
      try {
        const usdcContract = getContract({ client, chain: config.chain, address: USDC_ADDRESS, abi: ERC20_ABI });
        const balanceRaw = await readContract({ contract: usdcContract, method: "balanceOf", params: [address] });
