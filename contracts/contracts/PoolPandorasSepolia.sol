...
/**
 * @title PoolPandorasSepolia
 * @notice Versión para Sepolia/Testnet del Vault. 
 *         Permite configurar dirección de USDC en constructor.
 */
contract PoolPandorasSepolia is AccessControl, ReentrancyGuard, ERC2771Context {
    ...
    // --- Constantes y enums ---
    IERC20 public immutable usdc; // Immutable variable instead of constant
        ...
        ERC2771Context(trustedForwarder)
    {
        ...
        usdc = IERC20(_usdcAddress); // Set USDC address
    ...
    function depositUSDC(uint256 amount) external nonReentrant whenDepositsNotPaused {
        if (address(usdc) == address(0)) revert ZeroAddress(); // Prevent if mock usdc not set
        if (amount == 0) revert InvalidAmount();
